!function(doc) {
    ...
    var totalCountSpan = getElement('total-count'),
        todosCountSpan = getElement('todos-count'),
        completedCountSpan = getElement('completed-count'),
        todoEle = getElement('todo'),
        itemEle = getElement('item'),
        addButton = getElement('addItem'),
        markAllCompleted = getElement('all-completed'),
        clearAllCompletedBtn = getElement('clear-completed-btn'),
        showAllBtn = getElement('all'),
        showActiveBtn = getElement('active'),
        showCompletedBtn = getElement('completed'),  
    ...
    var model = {
        ...
        _clearAllCompleted: function() {
            var ItemsToDel = data.todos.filter(todo => todo.status !== 1);

            data.todos = ItemsToDel;
            localStorage.setItem('todoList', JSON.stringify(data));
            uiTodos = data.todos;
            this._refresh();
        },

        _sortIncrease: function() {
            data.todos.sort((a,b) => Number(b.id) - Number(a.id));
        },
        ...
        _showActive: function() {
            uiTodos = this._getActiveItems();
            this._refresh();
        },

        _showCompleted: function() {
            uiTodos = this._getCompletedItems();
            this._refresh();
        },
        ...
        _refresh: function(){
            this._sortIncrease();
            todoEle.innerHTML = '';
            uiTodos.forEach(todo => this._addItemToDOM(todo));
        ...
        _completedItem: function(todoId){
    ...
    _removeItem: function(todoId) {
        var itemRemove = data.todos.filter(todo => todoId !== todo.id);
        data.todos = itemRemove;
        localStorage.setItem('todoList', JSON.stringify(data));  
        uiTodos = data.todos;
        this._refresh();
    },
    _editItem: function(todoId) {
     ...
     _addItemToDOM: function(todo) {
        ...
        remove.addEventListener('click', this._removeItem.bind(model,todo.id));
    
        var complete = document.createElement('button');
        complete.classList.add('complete');
        complete.innerHTML = completeSVG;
        complete.addEventListener('click', this._completedItem.bind(model,todo.id));
    
        // Add click event for editing the item
        var edit = document.createElement('button');
        edit.classList.add('edit');
        edit.innerHTML = editSVG;
        edit.addEventListener('click', this._editItem.bind(model,todo.id));
