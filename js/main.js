!function(doc) {
    ...
    var totalCountSpan = getElement('total-count'),
        ...
        clearAllCompleted = getElement('clear-completed-btn'),
    ...
    var model = {
        ...
        removeItem: function(todoId) {
            console.log("It work ???");
            this.data.todos.filter(todo => todoId !== todo.id);
            localStorage.setItem('todoList', JSON.stringify(this.data));
        },
        ...
        remove.addEventListener('click', this.removeItem(todo.id));
    ...
    var getData = model._getData.bind(model);
        getData();
    // var addItem = model._addItem.bind(model);
    //     addItem();
    var buttonAdd = model._buttonAdd.bind(model);
    addButton.addEventListener('click', buttonAdd);

    var addItemtoDOMtree = model.addItemToDOM.bind(model);
    addItemtoDOMtree(todo);
